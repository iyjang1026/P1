...
class EllipseIsophote:
    ...
    def obj_mask(self, threshold=list, iter=int):
        ...
        mask=None
        for i in range(iter):
            masked, threshold, aper = obj_rej_mask(self.hdu, threshold1[i], self.hdr, ra, dec,npix=5,kernel_size=1,mask=mask, bkg_thrsh=True)
            mask = masked
        self.mask = mask
    ...
    def ellipse(self, modeling=True):    
        img = np.ma.masked_where((self.mask==1), self.hdu)#np.ma.masked_array(self.hdu, self.mask)#
        #plt.imshow(img, origin='lower');plt.show();sys.exit()
        ellipse = Ellipse(img, geometry=self.geo)
        isolist = ellipse.fit_image(sma0=self.h, minsma=self.h*0.001,integrmode='bilinear',linear=False,step=0.05, sclip=3.0, n_clip=5, fflag=0.3,fix_pa=True, fix_center=False) #isophote #maxsma=5*self.h,
...
ellipse.data_imp()
ellipse.obj_mask(threshold=[1.5], iter=1)
...
tbl = ellipse.ellipse(modeling=False)
radius = tbl['sma'] * 1.86
